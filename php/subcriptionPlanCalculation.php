...
function package($days,$priceForOneLiter){
    ...
    if ($result) {
        ...
        for ($i = 0; $i < $days; $i++) {
            ...
            $query = "INSERT INTO `orders`(O_ID, O_DATE, O_VOLUME,PRICE, DELEVERY_DATE, CUSTOMER_NAME, CONTACT_NO, ADDRESS, SR_ID,O_STATUS) VALUES ('','" . $date . "','". $_SESSION[' quntityInpackage']."','".$priceForOneLiter."','" . $deliverd_date . "','" . $_SESSION['u_name'] . "','" . $_SESSION['u_mobile'] . "','" . $_SESSION['u_address'] . "','Not Set' ,'pending')";
            $result = mysqli_query($con, $query);
            if (!$result) {
                die("NO query");
